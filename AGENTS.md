...
1. Skill 存放位置只有两种，不允许自创第三种：
   - 工程自有 Skills（视频处理相关，例如 ihatevideos-input、ihatevideos-export）放在仓库根目录 `skills/<Skill名>/`，随仓库提交。
   - 通用工具 Skills（例如 find-skills、git-small-project-workflow）放在本地 `.agents/skills/<Skill名>/`，不进仓库。
   - Skill 目录名必须和 SKILL.md frontmatter 中的 name 完全一致。
2. `.agents/`、`temp/`、`.venv/` 只存本地状态，不进仓库（以 .gitignore 为准）。
3. 没有明确指令不创建 Commit；一个任务只提交一个 Commit；整理未 push 的本地历史使用 reset 压缩。
4. 位置、范围、提交形态拿不准时，先用问题向用户确认再动手，不代做决定。

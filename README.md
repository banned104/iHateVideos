...
- `skills/`：随仓库提交的工程 Skills（`ihatevideos-input`、`ihatevideos-export`）。
- `.agents/skills/`：通用工具 Skills，只存本地，不提交。
- `temp/`：中间结果与 Cookie 文件，只存本地，不提交。
...
1. 在浏览器登录 B 站，用 Cookie 导出扩展（例如 Get cookies.txt LOCALLY）导出 Netscape 格式，把文件粘到 `temp/` 目录下，文件名随意。
2. 运行 `uv run ihatevideos-input cookies --file temp/<你粘的文件名>`（或设置 `BILIBILI_COOKIES_FILE` 后不传参数）。
